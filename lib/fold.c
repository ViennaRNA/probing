...
PRIVATE int   fill_arrays_pb(const char *sequence);
...
PRIVATE void update_perturbations(int length);
PRIVATE double get_perturbation(int i, int j);
...
PUBLIC float fold_pb(const char *string, char *structure){
  int i, length, energy, bonus=0, bonus_cnt=0;
...
PRIVATE int fill_arrays_pb(const char *string) {
  ...
  for (i = length-TURN-1; i >= 1; i--) { /* i,j in [1..length] */
    ...
    for (j = i+TURN+1; j <= length; j++) {
      ...
      if (type) {   /* we have a pair */
        ...
        new_c = (no_close) ? FORBIDDEN : E_Hairpin(j-i-1, type, S1[i+1], S1[j-1], string+i-1, P)+perturbations[i+1][j-1];
        ...
        for (p = i+1; p <= MIN2(j-2-TURN,i+MAXLOOP+1) ; p++) {
          ...
          for (q = minq; q < j; q++) {
            ...
            energy = E_IntLoop(p-i-1, j-q-1, type, type_2,
                               S1[i+1], S1[j-1], S1[p-1], S1[q+1], P) + perturbations[i+1][p-1] + perturbations[q+1][j-1];
